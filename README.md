...
- **[도서관 정보나루](https://data4library.kr)** (data4library): monthly loan counts from Seoul's public libraries, as the checkouts of the most-borrowed book, of a lower-ranked one, and of the top titles combined. Titles are never named, so the labels are bare ranks. The harvester is written and tested, but this source **has not yet produced a post**: the issued key has returned `vitalizationErr` ("the API is not in an activated state") on every call since it was granted, so the cache it reads has never been written. Left in place rather than removed, since nothing about it is wrong except the key. **Its monthly launchd job was unloaded and disabled on 21 August 2026**, after 35 days of firing against a dead key to no effect; the harvester and the vein are untouched and only the schedule is off. Seoul's own portal turns out to carry library loan counts on the key this bot already holds (`SeoulLibraryBookRentNumInfo`), which may replace this source entirely — the one thing missing there is a date or period field saying what window a count covers.
...
| `seoul_index_books_harvest.py` | Monthly harvest of Seoul public-library loan counts into `books_agg.json`, like the sales scan. Blocked on key activation, and its launchd job is disabled as of 21 Aug 2026 (see Data sources). |
...
- 도서관 정보나루 (`data4library_key`): Optional. The books harvester IS wired up and called on every run, but the issued key has returned `vitalizationErr` on every call since it was granted, so the vein has never produced a post. Without the key the bot still runs and those lines simply don't appear.
