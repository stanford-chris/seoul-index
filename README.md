...
- **[Seoul Open Data](https://data.seoul.go.kr), bus routes** (`CardBusStatisticsServiceNew`, per-stop boardings summed by route number, own "busroutes" category, own post): the day's busiest, second-busiest and quietest bus routes plus the day's total bus boardings — a complete four-line ranking, all four compulsory, own post, never mixed with the transport vein's subway/bus totals. Named by bare route number only ("Route 143"), never the full route name's Korean origin/destination. ⚠️ **The ranking is over numbered city routes only, and the footnote says so** ("Numbered city routes only" / "마을버스와 출퇴근 전용 노선 제외"), his call on 10 September 2026 after a measurement: 276 of the 664 route numbers in a day's feed carry Hangul (village buses prefixed with their district, "마포01", and rush-hour-only variants, "8442퇴근"), which have no safe English form to invent, and on **every weekday** 1-7 September the quietest route was 8442퇴근 at 17-23 boardings. The vein as first built withheld the whole card whenever any of the three ranked routes had no safe English form, and that guard was refusing it five days in seven; only weekend data got through (the 9 September post was Sunday 6 September's). The day's total still counts every route, so the caveat qualifies the ranking and not the sum. The all-or-nothing withhold stays as the belt, and now prints why when it fires. The busiest route has been the same one (Route 143) on every day checked as of 9 Sep 2026, so the vein tracks a day-over-day streak (`_bus_route_streak()`) and only mentions it in the footnote once it has actually run at least `BUS_ROUTE_STREAK_MIN` (3) consecutive published days — a fresh or broken streak says nothing rather than "led for the past 1 days". On a 3-day cooldown (`BUSROUTES_COOLDOWN_DAYS`) for the same reason as bike/traffic/transport/tourism: the busiest line rarely changes, so the vein hands back close to the same top line whenever it is offered. **The busroutes thread carries a fifth post: a route map** (`bus_route_map_stops()` in the poster, `render_bus_route_map()` in the card module, added 10 September 2026 after the 9 September post went out as the usual four with no map). It is drawn from data already in hand rather than linked: no reliable per-route URL exists on Kakao, Naver, TOPIS or bus.go.kr (each was checked), so the three ranked routes are plotted in red, amber and black over a faint silhouette of every Seoul bus stop, ordered by the bracketed sequence number each stop name carries in the boardings feed, the same trick `seoul-transit-art` already verified against real coordinates. ⚠️ The caption says so on the image: a route is shown as the stops where it saw a boarding that day, which for a quiet route is not its full official path. The stops are fetched at posting time, not harvest time, since keeping ~40,000 per-stop rows in a state file written every run for a vein on a 3-day cooldown was the wrong trade. ⚠️ The map is posted inside its own broad `except`, deliberately: the four-post thread is already public by then and the state write comes after, so an error escaping here would abort the run with the post out and `last_cat` and the cooldown unsaved, and the next run could repost the same card. A failed map costs a reply; a failed state write costs a duplicate thread.
