...
- **[Seoul Open Data](https://data.seoul.go.kr), library loans** (`SeoulLibraryBookRentNumInfo`): the checkouts of the most-borrowed book at Seoul Library, of a lower-ranked one, and of the top ten combined. Titles are never named, so the labels are bare ranks. **One library, the city's flagship, not the 215 public libraries counted elsewhere on these cards** — the same building the membership lines come from, and the counts are correspondingly small: 32 checkouts topped the list on 22 August 2026. The opener is required to name the library for exactly that reason.
  ...
  ⛔ **Do not diff two harvests to manufacture a calendar month.** The window rolls, so old loans fall out of it and a delta is net change rather than checkouts, and can go negative. Counts are also per RECORD, not per title: 모순 appears twice with different ISBNs at 30 and 9, and the library's own TOP 100 lists them separately rather than summing. Aggregating would break the one public cross-check this vein has.
...
| `seoul_index_books_harvest.py` | Weekly harvest of Seoul Library's 60-day loan counts into `books_agg.json`, like the sales scan. Re-reads the library's own published window every run and aborts if it cannot (see Data sources). |
