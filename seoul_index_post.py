...
OPENERS = [
    ...
    ('Through Gimpo International Airport', '김포공항에서'),
...
def station_gap_facts(c, d, d_ko):
    ...
    if not en:
        ...
        return []
    dow_dt = _day_dt(c['date'])
    stationgap_note_en, stationgap_note_ko = with_latest(
        STATIONGAP_NOTE_EN, STATIONGAP_NOTE_KO, d, d_ko, dow_dt.date())
...
#
# ⚠️ Narrowed 13 September 2026, his call: the sentence is now SKIPPED
# when the period is no more than one degree removed from today's own,
# necessarily-incomplete day/week/month -- yesterday for a date, the week
# or month that just ended for a week/month, which is as fresh as a period
# of that kind can possibly be given the data always lags by at least that
# much. Saying "the latest" there is trivially true and adds nothing: the
# KOPIS week card posted 13 September read "September 6 to 12", ending the
# day before the post, and stating that was obviously the newest available
# week told the reader nothing they couldn't already see. It still appears
# whenever a period is MORE than one degree removed (stations, four days
# behind on the same day), which is the case worth naming.
LATEST_UNITS = {'date': ('date', '날짜'), 'week': ('week', '주'), 'month': ('month', '달')}


def latest_is_notable(end_date, unit='date', today=None):
    """True when `end_date` -- the real last day a period covers -- is more
    than one degree removed from today's own, still-incomplete day/week/
    month: a fresher period of the same kind could, in principle, already
    exist. False when the period is already as fresh as one of its kind can
    possibly be (yesterday for a date, last calendar week for a week, last
    calendar month for a month), in which case latest_note()'s sentence is
    trivially true and is left off. A week's "one degree" is judged over 7
    days, not calendar weeks, since these are rolling windows (KOPIS, the
    busweekend span) rather than fixed Mon-Sun ones."""
    if today is None:
        today = datetime.now(SEOUL_TZ).date()
    if unit == 'date':
        return (today - end_date).days > 1
    if unit == 'week':
        return (today - end_date).days > 7
    if unit == 'month':
        return (today.year - end_date.year) * 12 + (today.month - end_date.month) > 1
...
def history_bus_facts(h, day, d, d_ko):
    ...
    if nb is None:
        ...
        RANKED_CARD_INFO['nightbus'] = {
            ...
            'note_ko': ('심야 노선만.'
                        + (f' {nb_latest_ko}.' if nb_notable else '')),
            'map_day': day, 'map_caption': caption,
    ...
    if ws is None:
        ...
        week = ws['week']
        ws_latest_en, ws_latest_ko = latest_clause(
            _span_en(week[0], week[6]), _span_ko(week[0], week[6]),
            _day_dt(week[6]).date(), 'week')
        RANKED_CARD_INFO['busweekend'] = {
            ...
            'note_en': ' · '.join(x for x in (
                'Saturday and Sunday per day against Monday to Friday per day',
                'trunk and branch routes over 1,000 weekday boardings',
                ws_latest_en) if x),
            'note_ko': ' · '.join(x for x in (
                '토·일 하루 평균 대 월~금 하루 평균',
                '평일 승차 1,000명 이상 간선·지선',
                ws_latest_ko) if x),
            'map_day': week[5],
...
def en_date_dow(dt):
    """'Wednesday, September 9': en_date() with the day of the week in
    front. Used only for the red masthead of a card whose dateline is a
    bare date and nothing else (stations, stationgap, seoulstation,
    wxday), his call, 13 September 2026, on seeing the stations card fly
    "September 9" with no sense of which day of the week that was. Every
...
def wx_day_facts(key):
    ...
    RANKED_CARD_INFO['wxday'] = {
        ...
        # The masthead carries the day of the week, his call, 13 September
        # 2026 (see en_date_dow()); day_en stays the bare date.
        'dateline_en': en_date_dow(yday), 'dateline_ko': ko_date_dow(d_ko, yday),
        'note_en': wxday_note_en,
        'note_ko': wxday_note_ko,
...
def kac_facts(key):
    ...
    facts = [fact('kac_pax_now', 'airport',
                  f'Passengers through Gimpo International Airport, {per_en}',
    ...
    if then:
        ...
        facts.append(fact('kac_pax_then', 'airport',
                          f'Passengers through Gimpo International Airport, {then_en}',
...
def rescue_facts(key):
    ...
    rescue_note_en, rescue_note_ko = with_latest(
        RESCUE_NOTE_EN, RESCUE_NOTE_KO, span_en, span_ko, end, 'week')
    RANKED_CARD_INFO['rescue'] = {
        ...
        'note_en': rescue_note_en,
        'note_ko': rescue_note_ko,
...
def kopis_facts(key):
    ...
    if any(k not in row for k in need) or not row['prfdtcnt']:
        ...
        return []
    span_en, span_ko = _span_en(a, b), _span_ko(a, b)
    kopis_note_en, kopis_note_ko = with_latest(
        KOPIS_NOTE_EN, KOPIS_NOTE_KO, span_en, span_ko, end, 'week')
    RANKED_CARD_INFO['kopis'] = {
        'day_en': span_en, 'day_ko': span_ko,
        'opener_en': KOPIS_OPENER_EN, 'opener_ko': KOPIS_OPENER_KO,
        'dateline_en': span_en, 'dateline_ko': span_ko,
        'note_en': kopis_note_en,
        'note_ko': kopis_note_ko,
...
_KEPCO_CACHE = {}
# Hong Kong's own annual residential figures, for the international
# comparison his ask added 13 September 2026 -- checked BY HAND, not
# fetched, and that is deliberate rather than an oversight. CLP Power
# (Kowloon and the New Territories, over 80% of Hong Kong's households)
# publishes its own ESG Databook as an .xlsx at clpgroup.com, and that
# host sits behind a WAF that answers a plain curl with a bare 403 --
# verified 13 September 2026 across several User-Agent and header
# combinations, all refused, while the identical request from a real
# browser succeeds every time. The Ticketlink entry in
# kbo_ticket_watch.py is the same shape: a source a launchd job cannot
# reach becomes a fixed, dated, hand-checked value rather than a silent
# gap. HK Electric (Hong Kong Island, the other ~16%) publishes a plain
# PDF curl reads fine, but there is little point building a fetch-and-
# parse pipeline for a fifth of a figure whose larger half must be read
# by hand anyway -- both are refreshed together, by hand, whenever the
# next editions of CLP's ESG Databook and HK Electric's Sustainability
# Report statistics PDF appear (both have run in April in every year
# checked so far).
#
# Combined, the two utilities cover essentially all of Hong Kong (~7.5
# million people), which is a cleaner match to "one city" than an
# American or Japanese utility territory would be: Con Edison's own
# reach (New York City plus Westchester County) and TEPCO's (all of
# greater Tokyo, ~45 million across eight prefectures) both spill well
# past the city itself, while CLP Power and HK Electric together ARE
# Hong Kong.
#
# 2025 figures (the newest full year each utility has published), read
# by hand 13 September 2026:
#   CLP Power -- ESG Databook 2025, 'Customers' sheet, 'Electricity
#     delivered' table: 2,528,522 residential customers, 9,966,000,000
#     kWh delivered.
...
# Kong-total comparison is confounded by nothing more than Seoul having a
# bigger population (9.3M against 7.5M) -- the same shape of mistake as the
# very first "share of the national total" proposal this session rejected.
# Per capita is the confound-free cut for a WHOLE-CITY total the way per
# household already is for the residential-only one.
#
# Hong Kong side, hand-checked 13 September 2026 (same refresh cadence and
# reasoning as HK_ANNUAL_* above -- these are official government figures,
# not the utilities', so a THIRD source, but still annual and still not
# reachable by an unattended fetch worth building for a once-a-year number):
#   Electricity: Census and Statistics Department, "Hong Kong Energy
#     Statistics Annual Report", 2025 edition, Table 4.1 "Local Consumption
#     of Electricity by Type of Users" -- 164,433 terajoules, ALL sectors
#     (domestic, commercial, industrial, street lighting), the government's
#     own total across every local electricity supplier, not just CLP Power
#     and HK Electric's own filings.
...
def _kepco_totals(rows):
    """Sum the month: {'cust', 'kwh', 'bill', 'house_kwh', 'house_cust',
    'general_kwh'}, or None when a row's numbers do not parse. house_cust
    is the 주택용 customer count alone (unlike 'cust', which is every
    contract type), so house_kwh / house_cust is an average per residential
    customer -- the figure the kepco card's own note used to say was
    deliberately NOT computed here, because houseAve.do publishes it
    per district for the kepcohouse card. Added 13 September 2026 so the
    Seoul side of the national comparison below is derived the same way
    the national side has to be (KEPCO publishes no national per-household
    average anywhere), rather than mixing a published Seoul figure with a
    derived national one."""
    t = {'cust': 0, 'kwh': 0, 'bill': 0, 'house_kwh': 0, 'house_cust': 0,
         'general_kwh': 0}
    for r in rows:
        try:
...
def _kepco_seoul_annual(key, end_y, end_m):
    ...
    can't be read -- a partial year must never be presented as an annual
    figure. 'kwh' is every contract type (feeds the per-capita pair);
    'house_kwh'/'house_cust' are 주택용 alone (feeds the per-household
    pair) -- both read off the SAME 12 fetches, so the per-capita and
    per-household comparisons are never quietly built from different
    months. house_cust is an average across the 12, not a single month's
    count, the same reasoning CLP Power and HK Electric's own annual
    figures rest on: a residential customer total drifts slowly across a
    year (Seoul's moved by about 3,200 of 2.9 million across the 12
    months checked 13 September 2026). Reuses _KEPCO_CACHE via
    _kepco_rows, so this costs 11 more calls than the current month
    already fetched for kepco_facts, once every KEPCO_COOLDOWN_DAYS --
    negligible."""
    total_kwh = 0
    total_house_kwh = 0
    cust_samples = []
    y, m = end_y, end_m
    for _ in range(12):
        rows = _kepco_rows(key, y, m)
        if not rows:
            return None
        t = _kepco_totals(rows)
        if not t or not t['house_cust']:
            return None
        total_kwh += t['kwh']
        total_house_kwh += t['house_kwh']
        cust_samples.append(t['house_cust'])
        m -= 1
        if m == 0:
            m = 12
            y -= 1
...
def _seoul_population(kosis_key):
    """Seoul's registered population (KOSIS, DT_1B040A3/T20, objL1 '11'), or
    None: no key, a KOSIS outage, or a row that doesn't parse. The same
    table kosis_facts() reads for 'pop_seoul', so the per-capita pair below
    and the 'national' vein's own population card can never disagree about
    what Seoul's population is. Only a SUCCESSFUL read is cached -- a
    failure returns None without touching the cache, so a keyless or
    failed call can never poison a later call made with a working key."""
    if 'seoul_pop' in _SEOUL_POP_CACHE:
        return _SEOUL_POP_CACHE['seoul_pop']
    if not kosis_key:
        return None
    from urllib.parse import quote
    try:
        row = _kosis_row(quote(kosis_key, safe=''), 'DT_1B040A3', 'T20', '11')
        result = int(row['DT'])
    except (RuntimeError, KeyError, ValueError, TypeError):
        return None
    _SEOUL_POP_CACHE['seoul_pop'] = result
    return result
...
def kepco_house_facts(key):
    ...
    per_en, per_ko = f'{MONTHS_EN[m - 1]} {y}', f'{y}년 {m}월'
    kepcohouse_note_en, kepcohouse_note_ko = with_latest(
        KEPCO_HOUSE_NOTE_EN, KEPCO_HOUSE_NOTE_KO, per_en, per_ko, date(y, m, 1), 'month')
    RANKED_CARD_INFO['kepcohouse'] = {
        ...
        'note_en': kepcohouse_note_en,
        'note_ko': kepcohouse_note_ko,
        'emoji': '⚡️'}
...
def seoul_station_facts(key):
    ...
    #
    # The plain-date case carries the day of the week, his call, 13
...
def rail_commuter_facts(gov_key, api_key):
    ...
    railcommuter_note_en, railcommuter_note_ko = with_latest(
        RAILCOMMUTER_NOTE_EN, RAILCOMMUTER_NOTE_KO, per_en, per_ko, date(y, m, 1), 'month')
...
def compose(sel, pool):
    ...
    # A cross-pair card mixing two DATED_PERIOD_CATS veins covering DIFFERENT
    # spans of time (a whole MONTH of visitors beside one DAY of admissions,
    # say) puts two spans of time on one card. per_pairs further down already
    # refuses to lift either as a single masthead dateline when they disagree
    # (correctly — neither span covers the whole card), but that left both
    # spans stranded together in one footnote line with nothing to say which
    # line either one explains:
    ...
    #
    # So any such pair groups like the weather then-and-now cards do: each
    # span drawn once as a subhead over the lines it covers, rather than a
    # single masthead making a claim that is only true of some of the rows
    # under it.
    #
    # Standing rule, 18 September 2026: any card carrying a date shows it on
    # a clear second line under the title — a dateline/masthead — never
    # buried only in the footnote. See the London Index counter-example this
    # generalises against, where a mixed-period card shipped with no
    # dateline at all: https://bsky.app/profile/london-index.bsky.social/post/3mvqfhjc3wh24
    #
    # This used to be hardcoded to the one pair that had actually shipped
    # (tourism/boxoffice), on the reasoning that generalising without a
    # second real example to test against would be guessing. There now is
    # one: `_validate_card_categories` only lets a DATED_PERIOD_CATS pair
    # share a card when `cross_vein_pairs()` found them a near-magnitude
    # coincidence, which needs a shared `unit` — and DATED_PERIOD_CATS
    # splits into exactly two such unit clusters, {tourism, boxoffice, rush}
    # ('people') and {property, spending, avgbill} ('won'), so
    # property+spending or property+avgbill is exactly as reachable as
    # tourism+boxoffice is (spending+avgbill themselves never disagree —
    # see SALES_Q below — so that pairing is not a case this needs to
    # handle). `period_group_entries` is the registry any such pair, current
    # or future, reads from.
    #
    # airport, incheon and rail are DATED_PERIOD_CATS members left OUT of the
    # registry, and that is not an oversight: their period is read off each
    # LINE once `lines` is built (kac_period/iiac_period/korail_period,
    # further down in this function), because their harvesters stamp a
    # period per FACT rather than into one shared module-level global the
    # way tour_facts()/boxoffice_facts()/molit_facts()/sales_facts()/
    # rush_facts() do — and `lines` is itself built from the picks ORDER
    # this very check decides, so their period cannot be known this early
    # without reordering the function, which is out of scope for this fix.
    period_group_entries = {}
    if TOUR_M['en']:
        period_group_entries['tourism'] = (
            TOUR_M['en'], TOUR_M['ko'],
    ...
    if BOXOFFICE_D['en']:
        ...
        period_group_entries['boxoffice'] = (
            BOXOFFICE_D['en'], BOXOFFICE_D['ko'],
            BOXOFFICE_D['en'], BOXOFFICE_D['ko'])
    if MOLIT_M['en']:
        # Same "descriptor, period" shape the footnote entry below carries
        # (see uses_molit) — reused verbatim as the subhead rather than
        # hand-worded like tourism/boxoffice, since a filing month has no
        # own-vein count claim to go stale the way boxoffice's does.
        period_group_entries['property'] = (
            MOLIT_M['en'], MOLIT_M['ko'],
            f'Apartment filings, {MOLIT_M["en"]}',
            f'아파트 실거래 신고, {MOLIT_M["ko"]}')
    if SALES_Q['en']:
        # spending and avgbill are two views of the SAME dataset (see
        # SALES_Q below) and always share its period, so pairing them with
        ...
        period_group_entries['avgbill'] = (
            SALES_Q['en'], SALES_Q['ko'],
            _spending_subhead_en, _spending_subhead_ko)
    if RUSH_M['en']:
        period_group_entries['rush'] = (
            RUSH_M['en'], RUSH_M['ko'],
    ...
    # identity string, position 0/1 of each entry) — without the second
    # check, two categories that happen to share one period (spending +
    # avgbill, always; property + spending on a month that lands in the same
    # quarter, occasionally) would be split into two redundant subheads
    # saying the same thing instead of being left to the ordinary single-
    # dateline lift further down.
    _pg_present = precats & set(period_group_entries)
    period_grouped = (
        not spotlight and not maybe_grouped and not metric_grouped
        and len(_pg_present) >= 2
        and len({period_group_entries[c][0] for c in _pg_present}) > 1)
    period_subheads = {}
    if period_grouped:
        for c in _pg_present:
            period_subheads[c] = period_group_entries[c][2:4]
    ...
    # same device tour_scope_pair/boxoffice_scope_pair used before this was
    # generalised, now keyed by category so any DATED_PERIOD_CATS member in
    ...
    if ('spending' in cats or 'avgbill' in cats) and SALES_Q['en']:
        ...
        if 'spending' in cats:
            dated_scope_pair['spending'] = _spending_pair
        if 'avgbill' in cats:
            dated_scope_pair['avgbill'] = _spending_pair
    ...
    if uses_molit:
        ...
        src_ko += ' · 국토교통부'
        if MOLIT_M['en']:
            _property_pair = (('Apartment filings', MOLIT_M['en']),
                              ('아파트 실거래 신고', MOLIT_M['ko']))
            scope_en.append(_property_pair[0])
            scope_ko.append(_property_pair[1])
            dated_scope_pair['property'] = _property_pair
    if uses_apqa:
    ...
    if 'rush' in cats and RUSH_M['en']:
        ...
        _rush_pair = (('The total monthly boardings during the designated hour',
                      RUSH_M['en']),
                     ('해당 시간대 승차 인원, 한 달 합계', RUSH_M['ko']))
        scope_en.append(_rush_pair[0])
        scope_ko.append(_rush_pair[1])
        dated_scope_pair['rush'] = _rush_pair
    if 'stations' in cats and STATION_DAY['en']:
        # Same shape as busroutes directly above, but with the day of the
        # week in front: his call, 13 September 2026, on seeing this card's
        # masthead fly a bare "September 9" with no sense of which day that
        # was. Only the masthead carries it -- the footnote's "is the
    ...
    if uses_tour:
        ...
        src_ko += ' · 한국문화관광연구원'
        if TOUR_M['en']:
            _tour_pair = (('Paid-admission sites', TOUR_M['en']),
    ...
    if uses_kobis:
        ...
        if 'boxhist' in cats:
            ...
            # Deliberately NOT captured into dated_scope_pair: this branch is
            # gated on cats == {'boxoffice'} above, i.e. boxoffice is the
            # card's ONLY category, so period_grouped (which needs 2+
            # DATED_PERIOD_CATS members) can never be true here and there is
            # nothing for it to strip.
        elif BOXOFFICE_D['en']:
    ...
    if period_grouped:
        # Every entry just captured above (tourism/boxoffice/property/
        # spending/avgbill/rush, whichever are actually on this card) is
        # about to head its own group as a subhead (see period_subheads /
        # _items below) instead of sitting inline in the footnote — drop it
        # here so the card does not say the same span twice. Only categories
        # period_group_entries actually promoted are captured in
        # dated_scope_pair in the first place, so this never strips a
        # descriptor that stayed in the footnote.
        for _pr_en, _pr_ko in dated_scope_pair.values():
            scope_en = [e for e in scope_en if e != _pr_en]
            scope_ko = [e for e in scope_ko if e != _pr_ko]
    ...
    def _stations_note():
        ...
        if STATION_DAY['en']:
            en, ko = latest_clause(STATION_DAY['en'], STATION_DAY['ko'],
